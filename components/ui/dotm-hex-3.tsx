    ...
    >
      ...
      >
        {ROW_COUNTS.map((count, row) => (
          <div
            // biome-ignore lint/suspicious/noArrayIndexKey: fixed hex grid; row index is the identity
            key={row}
            style={{
              display: "flex",
              justifyContent: "center",
          ...
          >
            {Array.from({ length: count }).map((_, col) => {
              ...
              return (
                <span
                  // biome-ignore lint/suspicious/noArrayIndexKey: fixed hex grid; row,col is the identity
                  key={`${row},${col}`}
                  aria-hidden="true"
                  className={cx(
                    "dmx-dot",
