 ...
 *
 * Self-hosted: there is no cloud cross-encoder, so reranking scores candidates
 * locally with the same BM25-shaped measure `lexicalShortlist` uses.
 */
...
/**
 * Cheap lexical shortlist, run before the final ranking.
 *
 * The ranker is the accurate scorer but costs proportional to how much you hand
 * it — 216 passages measured at ~2.2s. This trims the field in-memory first
 * (microseconds) so the expensive scorer only sees plausible candidates.
 *
 ...
 * shortlist, it returns everything and lets the ranker do the real work —
...
export function lexicalShortlist<T>(
  ...
  // the source. Hand the ranker the full field rather than nothing.
  ...
  // spent either way, and the ranker gets to see them.
...
/**
 * Rank candidates by relevance to the query. Returns the top-N candidates
 * paired with a relevance score, most relevant first.
 *
 * The stock build sent this to a cloud cross-encoder (cohere rerank via the
 * Vercel AI Gateway). Fully self-hosted, this scores the (already shortlisted)
 * candidates locally with the same BM25-shaped measure above — rare query
 * terms weigh more, term frequency saturates — which keeps document search
...
export async function rerankByRelevance<T>(
  ...
  const terms = [...new Set(tokenize(query))]
  if (terms.length === 0)
    return candidates.slice(0, topN).map((item) => ({ item, score: 1 }))
  ...
  const scored = tokenized
    .map((tokens, i) => {
      ...
      return { item: candidates[i], score }
    })
