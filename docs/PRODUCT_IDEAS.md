# MiniScira backlog

This file contains only raw ideas and draft PRDs that need approval. Follow the lifecycle in [Product planning and execution](PRODUCT_PLANNING.md).
Remove an entry after the user explicitly approves its PRD.

## Backlog

### Artifact Library across all chats

[Draft PRD](../tasks/prd-artifact-library.md). Awaiting approval.

### Edit uploaded images with natural-language instructions

[Draft PRD](../tasks/prd-image-editing.md). Awaiting approval.

### Generate editable documents and presentations

[Draft PRD](../tasks/prd-document-generation.md). Awaiting approval.

### Clone and analyze GitHub repositories

[Draft PRD](../tasks/prd-github-repository-analysis.md). Awaiting approval.

### Archive and recover chat threads

[Draft PRD](../tasks/prd-thread-archiving.md). Awaiting approval.

### Search and read previous threads

[Draft PRD](../tasks/prd-thread-search.md). Awaiting approval.

### Nightly memory extraction from the day's chats

[Draft PRD](../tasks/prd-nightly-memory-extraction.md). Awaiting approval.
